...
#pragma region OPRF
			...
			//	for (u64 i = 0; i < numOPPRFs; i += stepSize)
		{
			...
			for (u64 k = 0; k < currStepSize; ++k)
				...
				{
					cout << "oprf-- " << i + k << ": " << encoding1[k] << endl;
...
void usage(const char* argv0)
{
		...
		std::cout << "\t\t Sender terminal: " << argv0 << " 0 16 8" << std::endl;
		std::cout << "\t\t Receiver terminal: " << argv0 << "1 16 8" << std::endl;
		std::cout << "\t\t (n=16, m=8)" << std::endl;
}
...
#pragma region SetupInput
		...
		cout << "matching pos: " << posMatches[0] << ",\t" << posMatches[1] << "\n";
...
#pragma region SetupInput
		//u64 numBlkText = atoi(argv[2]); 
		//u64 numBlkPattern = atoi(argv[3]);

		u64 numBlkText = 1 << (atoi(argv[2]) - 7);
		u64 numBlkPattern = 1 << (atoi(argv[3]) - 7);
